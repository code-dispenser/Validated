...
internal class Different_Tenant_And_Culture
{
    public static async Task Run(IValidatorFactoryProvider validatorFactoryProvider)
    {
        ...
        */
        ...
        /*
            * The Family name rule is comprised of two rules one for characters and one for length, lets in effect override the length one by adding a rule for a specific TenantID with a specific culture
            * And just to show the faillback lets add another rule that is for our TenantID without the culture os if there is not culture match it will just fallback to the tenant match
        */

        ruleConfigs = ruleConfigs.AddRange([
                                            new ValidationRuleConfig("Validated.RuleConfigurations.ConsoleClient.Common.Models.ContactDto", "FamilyName", "Surname", "RuleType_StringLength", "", "",
                                            "My French Tenant in French - Must be between 2 and 50 characters long", 2, 50, TenantID: "MyFrenchTenant", CultureID: "fr-FR"),

                                            new ValidationRuleConfig("Validated.RuleConfigurations.ConsoleClient.Common.Models.ContactDto", "FamilyName", "Surname", "RuleType_StringLength", "", "",
                                            "My French Tenant in English - Must be between 2 and 50 characters long", 2, 50, TenantID: "MyFrenchTenant")//just our tenant no culture
                                           ]);
        ...
        var getTenantIDFromSomeContext = "MyFrenchTenant";

        /*
            * We want rules for the TenantID of MyFrenchTenant with a culture of fr-FR. If there is not tenant/culture match it would fallback to the tenant/culture of ALL/en-GB 
        */
        ...
        */
        ...
        getCultureFromSomeContext = "NotFrenchCulture";
