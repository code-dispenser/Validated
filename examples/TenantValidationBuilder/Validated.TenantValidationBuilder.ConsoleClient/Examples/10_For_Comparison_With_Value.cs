...
{
    internal static class For_Comparison_With_Value
    {
        /*
            * ForComparisonWithValue is for a comparison validation between a member and a value in the configuration data
            * using one of the comparison types such as not equal to or less than or equal to
        ...
        */
        public static async Task Run(IValidatorFactoryProvider validatorFactoryProvider)
        {
            ...
            var contactValidator = TenantValidationBuilder<ContactDto>.Create(ruleConfigs, validatorFactoryProvider)
                                    .ForMember(c => c.Title)
                                        .ForComparisonWithValue(c => c.Age)//This is the left hand-side, the config entry is the right-hand-side
                                            .Build();

            Console.WriteLine("Executing the validator with a contact that has an age over 18.\r\n");
            ...
            Console.WriteLine("Executing the validator with a contact that has an age under 18\r\n");
