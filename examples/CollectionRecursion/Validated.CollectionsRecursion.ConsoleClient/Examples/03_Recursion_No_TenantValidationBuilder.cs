...
public static class Recursion_No_TenantValidationBuilder
{
    /*
    ...
    */
    ...
    {
        /*
            * Scenario is that you want to validate some tree structure that is N-Level deep, without using the TenantValidationBuilder. 
        */
        ...
        var nodeValidator       = validatorFactoryProvider.CreateValidator<string>(typeof(Node).FullName!, nameof(Node.Name), ruleConfigs);
        var nodeEntityValidator = ValidatorExtensions.ForEntityMember<Node, string>(nodeValidator, node => node.Name);

        var validator = ValidatorExtensions.ForRecursiveEntity<Node>(node => node.Child!, nodeEntityValidator);
